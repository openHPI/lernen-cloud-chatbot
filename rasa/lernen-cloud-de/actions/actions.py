...
class ActionGetCourses(Action):
	...
	def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
		...
		if status == 200:
			...
			if len(response) < 1:
				dispatcher.utter_message('Sie sind derzeit in keinen Kursen eingeschrieben.')
...
class ActionGetCourses(Action):
	...
	def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
		...
		if status == 200:
			response = json.loads(r.content)
			if len(response) < 1:
				dispatcher.utter_message('Sie sind derzeit in keinem Kursen eingeschrieben.')
				return [SlotSet('courses_available', False)]
			else:
				dispatcher.utter_message('Sind sind derzeit in diesen Kursen eingeschrieben:')
				for course in response:
					title = course['title']
...
class ActionGetAchievements(Action):
	...
	def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
		...
		if courseId != 0:	
			...
			dispatcher.utter_message('Es tut mir sehr leid! Ich konnte den Kurs, den Sie suchen, nicht finden. Bitte versuchen Sie es erneut, in dem Sie mir den Kurstitel nennen.')
			return[SlotSet('current_course_achieved', course_achieved), 
			SlotSet('current_course', currentCourse), 
			SlotSet('current_achievements', currentAchievements), 
			SlotSet('current_course_title', None)]
